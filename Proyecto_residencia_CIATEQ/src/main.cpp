...
IPAddress ip(192, 168, 0, 192); //*ip arduino
...
void loop() {
  ...
  if(command == 1){ //*Modo generador 
    ...
    if(start2){ //*inicio de monitoreo 
      //! Parte critica, ya que usa interrupciones 
      static int pos = 0;
      static int milis=0;
...
void serialEvent (){ //*lee la cadena proveniente de visual studio HMI 
  ...
  if(StringComplete){  //*validar comando recibido 
    ...
    if(inputString.length() == 4){
      if(inputString =="Init"){ //* String para saber si arduino se encuentra conectado a GUI
      inputString =""; 
      tiempo(300);
      Serial.println("123Z"); //*Respuesta "I'm alive" de arduino a HMI
      }
    } 
      ...
      else if (inputString =="C2"){  //*sin activación, se pierde comunicación con GUI 
        inputString ="";
        digitalWrite(LED_BUILTIN, LOW);
        command = 0;  
      }
      ...
      separar_ajustes_modbus(inputString); //*Separar paquetes de datos  
...
void Poller(){ //*secuencia de modbus registros de monitoreo: velocidad,Driver En, emergency Stop 
  ...
  if(acc == 360){
    Mb.MbData[0] = 0; 
    Mb.Req(MB_FC_READ_REGISTERS,  221,1,0); //*Solicitud de lectura registro DRV.ACTIVE 221
  }
  if(acc == 480){
    STATUS_drv = Mb.MbData[0]; //*Tomar dato  de Status driver 
  }
  
  if(acc == 600){
    Mb.MbData[0] = 0; 
    Mb.Req(MB_FC_READ_REGISTERS,  1055,1,0); //*Solicitud de lectura registro DRV.HWENABLE 1055
  }
  if(acc == 720){
    STATUS_stop = Mb.MbData[0]; //*Tomar dato de Status Emergency Stop (Hardware)
    acc =0; 
    start = true;  //*fin 
  }
} //*Poller_monitoreo general modbus  
...
void Poller_2(){  //*Registros de Axis Servomotor
  ...
  if(stop_axis){  //*si se va activar, entonces 
    if(acc == 350){
      Mb.MbData[0] = 1; 
      Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,  254,1,0); //*Escribir en registro DRV.EN
    }
    if(acc == 550){
      Mb.MbData[0] = 0; 
      Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,  254,1,0); //*Escribir en registro DRV.EN 
    ...
    if(acc == 350){
      Mb.MbData[0] = 1; 
      Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,  236,1,0); //*Escribir en regisro DRV.DIS
    }
    if(acc == 550){
      Mb.MbData[0] = 0; 
      Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,  236,1,0); //*Escribir en registro DRV.DIS
      acc = 0 ;
      iniciar_set2 = true;  //*salimos del loop 
    }
  }
...
void Poller_4(){  //*Registros de Movimiento Servomotor
  ...
  if(stop_move){ //*si se va a iniciar movimiento, entonces 
    if(acc == 350){
      Mb.MbData[0] = 1; 
      Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,  752,1,0); //*Escribir en registro SM.MOVE
    }
    if(acc == 550){
      Mb.MbData[0] = 0; 
      Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,  752,1,0); //*Escribir en registro SM_MOVE
      acc = 0 ;
    ...
    if(acc == 350){
      Mb.MbData[0] = 1; 
      Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,  274,1,0); //*Escribir en registro DRV.STOP
    }
    if(acc == 550){
      Mb.MbData[0] = 0; 
      Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,  274,1,0); //*Escribir en registro DRV.STOP
...
void Poller_3(){ //*Configurar registros de servodrive modbus 
  delay(1);
  acc = acc + 1; 
  
  if(acc ==120 ){ 
    Mb.MbData[0] = num1; 
    Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,  751,1,0); //*Escribir en registro SM_MODE 751 (tipo de movimiento)
  } 

  if (acc == 240){ 
    Mb.MbData[0] =  num2;  
    Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,  758,1,0);  //*Escribir en registro SM.V1 758 (Velocidad 1) 
  }

  if (acc == 360){ 
    Mb.MbData[0] =  num3;  
    Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,  760,1,0);  //*Escribir en registro SM.V2 760 (Velocidad 2)
  }

  if (acc == 480){ 
    Mb.MbData[0] =  num4;  
    Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,  218,1,0);  //*Escribir en registro ACC 218 (limíte de acceleración, no viene en manual)
  }

  if (acc == 600){ 
    Mb.MbData[0] =  num5;  
    Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,  232,1,0);  //*Escribir en registro DECC 232 (Límite de desaceleración, no viene en manual)
  }

  if (acc == 720){ 
    Mb.MbData[0] =  num6;  
    Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,  755,1,0);  //*Escribir en registro Timer1 755 (temporización 1) 
  }

  if (acc == 840){ 
    Mb.MbData[0] =  num7;  
    Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,  757,1,0);  //*Escribir en registro Timer2 757 (temporización 2)
  }

  if (acc == 960){ 
    Mb.MbData[0] =  num8;  
    Mb.Req(MB_FC_WRITE_MULTIPLE_REGISTERS,  235,1,0);  //*Escribir en registro SM.dir 235 (Sentido de giro)
    iniciar_set = true; //*sale del loop
    acc=0; 
  }
}
...
float analog_torque_voltage(){
  ...
  return(PromLectura); //*devuelve valor de lectura 
}
